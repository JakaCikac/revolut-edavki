...
def load_data():
    ...
    transactions["Price per share"] = transactions["Price per share"].apply(
        clean_amount
    )
...
def create_kdvp_xml(transactions, year, tax_number, taxpayer_type="FO"):
    ...
    stock_transactions = transactions[
        (transactions["Type"].str.contains("BUY|SELL", na=False))
        & (~transactions["Ticker"].isin(["VWCE"]))
    ]

    # Get tickers with sells in target year
    sells_in_year = stock_transactions[
        (stock_transactions["Type"].str.contains("SELL", na=False))
        & (stock_transactions["Date"].dt.year == year)
    ]
    ...
    for ticker in tickers:
        ...
        for _, tx in ticker_transactions.iterrows():
            ...
            # Convert amount
            orig_price, eur_price, rate = clean_amount(
                tx["Price per share"], fx_rate=fx_rate, return_details=True
            )
...
def create_div_xml(transactions, company_info, year, tax_number, taxpayer_type="FO"):
    ...
    if len(company_info) == 0:
        ...
        company_info = pd.DataFrame(
            columns=["Symbol", "Name", "Address", "CountryCode", "ISIN"]
        )
    ...
    for _, tx in dividend_transactions.iterrows():
        ...
        # Convert amount
        orig_amount, eur_amount, rate = clean_amount(
            tx["Total Amount"], fx_rate=fx_rate, return_details=True
        )
        print(
            f"Amount: {eur_amount:.2f} EUR (original: {orig_amount:.2f} {currency}, rate: {rate})"
        )
        ...
        ET.SubElement(div_item, "ForeignTax").text = (
            "0.00"  # You might want to update this based on actual data
        )
