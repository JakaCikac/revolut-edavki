...
def process_files(transactions_file, company_info_file, year, tax_number, taxpayer_type="FO"):
    try:
        ...
        for ticker in sell_tickers:
            ...
            sells = ticker_tx[(ticker_tx["Type"].str.contains("SELL", na=False)) & (ticker_tx["Date"].dt.year == year)]
        ...
        div_tx = transactions_year[
            (transactions_year["Type"] == "DIVIDEND") & (~transactions_year["Ticker"].isin(["VWCE"]))
        ]
...
def health():
    ...
    return jsonify({"status": "healthy", "service": "revolut-edavki", "version": "1.0.0"}), 200
...
def upload():
    ...
    try:
        ...
        return (
            jsonify({"error": "An error occurred processing your files. Please check the format and try again."}),
            500,
        )
